...
namespace :postgresql do
  ...
  task :create_database, roles: :app, only: {primary: true} do
    run %Q{#{sudo} dpkg-reconfigure locales}
    run %Q{#{sudo} -u postgres psql -c "create user #{postgresql_user} with superuser createdb encrypted password '#{postgresql_password}';"}
